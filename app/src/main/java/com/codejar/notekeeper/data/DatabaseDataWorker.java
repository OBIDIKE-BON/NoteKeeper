...
public class DatabaseDataWorker {
    ...
    private void insertCourse(String courseId, String title) {
        ...
        values.put(NoteKeeperContract.CourseInfoEntry.COLUMN_COURSE_ID, courseId);
        values.put(NoteKeeperContract.CourseInfoEntry.COLUMN_COURSE_TITLE, title);

        long newRowId = mDb.insert(NoteKeeperContract.CourseInfoEntry.TABLE_NAME, null, values);
    }

     private void insertNote(String courseId, String title, String text) {
        ContentValues values = new ContentValues();
        values.put(NoteKeeperContract.NoteInfoEntry.COLUMN_COURSE_ID, courseId);
        values.put(NoteKeeperContract.NoteInfoEntry.COLUMN_NOTE_TITLE, title);
        values.put(NoteKeeperContract.NoteInfoEntry.COLUMN_NOTE_TEXT, text);

        long newRowId = mDb.insert(NoteKeeperContract.NoteInfoEntry.TABLE_NAME, null, values);
    }
